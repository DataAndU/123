...
import com.gemmaassistant.app.assistant.VoiceInputManager
import com.gemmaassistant.app.core.AppContainer
import com.gemmaassistant.app.data.ChatMessageEntity
...
fun ChatScreen(container: AppContainer) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(12.dp)) {
        Text(
            if (container.localLlmEngine.isLoaded()) {
                "Gemma model loaded — ask anything, including reading files or the web if you've granted access."
            } else {
                "No local model loaded yet. Load one in Settings for real understanding, or try a plain command like \"call mom\" or \"open camera\" right now."
            },
