...
// __android_log_is_loggable() needs API 30+; this app's minSdk is 26, so we
// do the same threshold check by hand instead (logcat's own runtime filters
// still apply on top of this either way).
...
static inline void llama_bridge_android_log_callback(enum ggml_log_level level,
                                                       const char* text,
                                                       void* /*user*/) {
